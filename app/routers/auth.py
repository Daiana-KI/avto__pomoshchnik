...
async def forgot_password_form(request: Request):
    ...
    html = jinja_env.get_template("forgot_password.html").render({})
...
async def forgot_password_sent(request: Request):
    ...
    return HTMLResponse("""
    <html>
    <head><title>Ссылка отправлена</title></head>
    <body>
...
async def reset_password_form(request: Request, token: str, db: AsyncSession = Depends(get_db)):
    ...
    html = jinja_env.get_template("reset_password.html").render({"token": token})
    return HTMLResponse(content=html)
